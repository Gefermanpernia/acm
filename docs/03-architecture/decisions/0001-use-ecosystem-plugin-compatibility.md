...
## Decision
Adopt the simplest viable convention: declare `@opencode-ai/plugin` as `^1.18.18`, retain Linux, ACM-profile, and credential-shape hard gates, and treat best-effort Claude CLI detection as diagnostics only. Validation is the production entry-point load plus focused and full Node suites. Every future superseding compatibility decision MUST amend its specification in the same slice.
