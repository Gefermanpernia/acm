...
Confirmed quota requires 429 + typed `rate_limit_error` + rejected unified-quota headers. ACM serializes transitions per profile and excludes profiles already recorded for the operation. Stale non-replay transitions MUST be rejected. ACM MAY accept a stale submitted generation only for a same-operation, same-profile ledger replay; it MUST return the current generation and MUST return before state persistence. This ledger replay returns the existing transition result before `saveMachineState`; a different operation or profile still follows normal validation and generation checks. Replacement available: plugin returns bounded 429 without provider `Retry-After`, prompting SessionRetry. Only cooling profiles: 429 with earliest-reset `Retry-After`. All quarantined: non-retryable 401 with `acm login` action and no retry header. Unconfirmed 401/429/529 pass through unchanged.
